...
class TestInstrumentWatcher:
	...
	def test_no_timer_scheduled_after_stop (self, tmp_path: pathlib.Path) -> None:
		...
		"""A debounce/retry callback landing on a scheduling site after stop()
		must not register a fresh timer that would outlive the watcher (the
		stop-vs-reschedule race).  The _stopped guard makes the sites no-op.
		"""
		...
		# Drive the two observer-thread scheduling entrypoints post-stop; each
		# should hit the _stopped guard and add nothing.
		watcher._on_sidecar_event(tmp_path / "late.wav.analysis.json")
		watcher._on_audio_file_event(tmp_path / "late.wav")
...
class TestInstrumentWatcherAudioPath:
	...
	def test_wav_without_sidecar_triggers_callback (self, tmp_path: pathlib.Path) -> None:
		...
		assert sidecar.exists(), "Sidecar should have been generated by ensure_sample_assets"

	def test_stale_sidecar_does_not_short_circuit_load (self, tmp_path: pathlib.Path) -> None:

		"""A WAV appearing next to a STALE sidecar (one whose stored MD5 is for
		different bytes) still triggers the audio-path callback: the freshness gate
		(sidecar_matches_audio) rejects the stale sidecar, so the new audio is
		re-analysed and loaded rather than silently skipped.
		"""
		...
		try:
			# A valid current-version sidecar, but its stored MD5 (a fixed fake)
			# will not match the WAV written next — i.e. a stale left-behind one.
			_write_sidecar(tmp_path, "snare")
			tests.helpers._make_wav(tmp_path / "snare.wav")
			triggered = done.wait(timeout=_AUDIO_PATH_TIMEOUT)
		finally:
			watcher.stop()
			_restore_audio_timings(originals)

		assert triggered, "stale sidecar wrongly short-circuited the audio load"
		assert received[0].name == "snare"
		assert received[0].audio is not None

	def test_with_preview_generates_png (self, tmp_path: pathlib.Path) -> None:

		"""A runtime-dropped audio file with previews enabled gets its .preview.png,
		matching the startup load — closes the gap where the watcher's audio path
		used to call load_or_analyze (no preview) instead of ensure_sample_assets.
		"""
		...
		watcher = self._make_watcher(
			tmp_path, lambda record: done.set(), with_preview=True,
		)
		watcher.start()

		try:
			tests.helpers._make_wav(tmp_path / "clap.wav")
			triggered = done.wait(timeout=_AUDIO_PATH_TIMEOUT)
		finally:
			watcher.stop()
			_restore_audio_timings(originals)
	...
	def test_dropped_audio_loads_exactly_once (self, tmp_path: pathlib.Path) -> None:

		"""The audio path's own sidecar write must not re-trigger the sidecar
		path — a dropped file was previously analyzed and integrated TWICE
		(the second delivery arriving via the freshly written sidecar).
		"""
		...
		try:
			...
			assert first.wait(timeout=_AUDIO_PATH_TIMEOUT), "Callback never called"

			# Give the self-written sidecar's (suppressed) event time to
			# debounce and fire — a second delivery would land here.
			time.sleep(subsample.watcher._DEBOUNCE_SECONDS + 1.0)
		finally:
			watcher.stop()
			_restore_audio_timings(originals)
...
class TestMidiMapWatcher:
	...
	def test_no_callback_or_timer_after_stop (self, tmp_path: pathlib.Path) -> None:
		...
		"""After stop(), a late file event must not schedule a new debounce
		timer, and a _fire() that was already in flight must not invoke the
		reload callback into a torn-down player.
		"""
		...
		# Late observer-thread event: must not schedule a timer.
		watcher._on_file_event(midi_map_path)
